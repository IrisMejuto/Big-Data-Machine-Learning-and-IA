...
```{r}
# Calcular porcentaje de apartamentos sin metros cuadrados
...
8.  Existen varios Barrios que todas sus entradas de Square.Meters son NA, vamos a eliminar del dataset todos los pisos que pertenecen a estos barrios.
    ...
    barrios_todo_na <- df_madrid |>
      ...
      summarise(
        num_na = sum(is.na(Square.Meters)),
        total_entradas = n()
      ) |>
...
# Rellenamos matriz con p-valores
...
13. **Vamos a crear una nueva columna en el dataframe df_madrid con un nuevo identificador marcado por los clusters obtenidos. Esta columna la llamaremos neighb_id**
    ...
    ```{r}
    neighborhood_list <- unique(df_madrid$Neighborhood)

    # Create the 'df_cluster' dataframe
    df_cluster <- data.frame(Neighborhood = neighborhood_list, neighb_id = factor())

    # Merge the 'df_madrid' and 'df_cluster' dataframes
    df_madrid <- merge(df_madrid, df_cluster, by = "Neighborhood", all=T)
    ```

    ```{r}
    unique(df_madrid$Neighborhood)
    ```
...
-   latitud y longitud
    ...
    y usar neighb_id como identificador
